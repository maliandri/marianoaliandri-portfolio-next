...
GROQ_API_KEY                        ← Solo GitHub Actions (bot de noticias), NO en Vercel.
                                       Secret en GitHub → Settings → Secrets and variables →
                                       Actions. Free tier sin tarjeta. Fallback de Gemini en
                                       scripts/noticias-bot.mjs (modelo openai/gpt-oss-120b)
...
- **Bot de noticias** (GitHub Actions, no Vercel): un script autocontenido (`scripts/noticias-bot.mjs`) busca noticias nuevas de los tópicos activos (`noticias_topics`) vía Google News RSS, le pide a Gemini que redacte título/cuerpo/caption y después arma la imagen: una **tarjeta 1080x1350 (4:5)** dibujada por `scripts/noticiaCard.mjs` con Satori + resvg (gratis, corre en Actions; tipografías Inter en `scripts/fonts/`) que lleva el tópico, el titular de la IA, el medio y `marianoaliandri.com.ar`. Cada tópico decide desde el admin (checkbox "Usar la foto del artículo", campo `usarFoto` en `noticias_topics`, default `true` si no existe) si la tarjeta lleva de fondo la foto del artículo (`og:image` vía Microlink `meta=true`, descargada con User-Agent de navegador porque varios diarios dan 403 sin él; si mide menos de 600px, no es JPEG/PNG/WebP o falla, cae al fondo de marca) o siempre el fondo de marca. El color de cada tópico sale de una paleta de 6 según su id. Antes se sacaba una captura de pantalla de la página (`screenshot=true`), que salía recortada y con banners — se eliminó (2026-09-21). Consecuencia: la imagen ya no descarta noticias (siempre hay tarjeta), por eso el texto se genera antes que la imagen. Instagram solo acepta JPG, así que a Make va la URL de Cloudinary con `f_jpg,q_auto` (sin recorte, la tarjeta ya es 4:5). Cada nota guarda `imageMode: 'foto' | 'marca'`. El `body` tiene un tope de **500 caracteres** (`MAX_BODY_CHARS` en el bot, decisión del usuario 2026-09-21; el prompt pide ≤450 y `fitBody()` recorta en un final de párrafo/oración si la IA se pasa; el texto compartido con Instagram tampoco puede superar 2.200, error 36004). El prompt exige que el `body` cuente la noticia completa (qué/quién/cuándo/por qué importa) — nada de gancho vacío tipo "enterate haciendo click"; el link a `/noticias/{id}` va aparte, como extra opcional, no como única forma de leer la nota (decisión 2026-09-18, hartazgo explícito con portales que hacen eso). **Fallback de proveedor**: si falla Gemini (error de red, contenido bloqueado, cuota agotada — la cuota de `GEMINI_API_KEY` es compartida con el resto del sitio, ver advertencia en el código) y hay `GROQ_API_KEY` configurada, reintenta con Groq (`openai/gpt-oss-120b` — Groq deprecó `llama-3.3-70b-versatile` para free tier el 2026-06-17, ver comentario en el código —, free tier sin tarjeta, mismo prompt). Cada doc en Firestore guarda `contentProvider: 'gemini' | 'groq'` para poder ver si el fallback se está usando seguido (señal de que Gemini se está quedando sin cuota). Sin `GROQ_API_KEY`, el bot se comporta como antes (solo Gemini, esa noticia se descarta si falla). Publica en `/noticias` + al mismo webhook de Make (`type: 'noticia'`) como post nativo en Facebook e Instagram con el cuerpo completo + link a `/noticias/{id}` (en Instagram ese link no es clickeable, limitación de la plataforma). **Destino por tópico** (`destino` en `noticias_topics`, default `fb_ig`; se elige en el admin): `fb_ig` = Facebook + Instagram, `linkedin` = SOLO LinkedIn (para tópicos de trabajo/empleo/trabajo remoto), `todas` = las tres, `x` = SOLO X (pensado para política, decisión del usuario 2026-09-21: **esas notas NO se muestran en el portfolio**). El bot lo traduce con `networksFor()` al campo `networks` del payload a Make. Los destinos que no son `x` publican además en `/noticias`; con `x` la nota se guarda igual en Firestore (dedup, tope diario, log del admin) pero con `visibleEnSitio: false`, y `/noticias`, `/noticias/[id]` (404) y `/api/noticias?public=1` la filtran. `GET /api/noticias` sin `?public=1` (log del admin) ahora **exige login de admin** (antes devolvía todo a cualquiera). Para `x` el bot no genera tarjeta, no usa Cloudinary y **no pasa por Make** (Make sacó su módulo de X): publica directo con la API de X (`scripts/xClient.mjs`, OAuth 1.0a, `POST /2/tweets`) el texto de `buildXText()` (la `caption` + el link final del artículo vía Microlink, ≤280 caracteres). Necesita los secrets de GitHub `X_API_KEY`, `X_API_SECRET`, `X_ACCESS_TOKEN`, `X_ACCESS_SECRET` (app de developer.x.com con permisos Read and Write, tokens generados después de darlos). Si X rechaza (cuota/pago) el error queda en `makeError` de la nota como `X: ...`. Sin verificar en una publicación real todavía. **Tope de notas por corrida por tópico** (`maxPorCorrida`, 1-5, default 2 = `MAX_ITEMS_PER_TOPIC`; `itemsPerRunFor()`), siempre dentro del máximo de 5 por corrida. **LinkedIn**: el módulo se había borrado de la rama "Noticia (bot)" de Make el 2026-09-20 porque las capturas recortadas daban vergüenza; con las tarjetas nuevas vuelve, pero segmentado por `destino`. Para que funcione la rama de Make necesita un filtro por `networks.facebook / instagram / linkedin` en cada módulo (FB, IG y LinkedIn en rutas de un Router separado, así un filtro en uno no corta a los otros). **Orden de despliegue**: primero el bot (que manda `networks` real), después Make — el bot viejo mandaba las tres en `true`, y un módulo de LinkedIn filtrado por `networks.linkedin` publicaría TODO en LinkedIn. Esa rama (módulos 50 = Facebook, 52 = Instagram) tiene un handler `builtin:Ignore` en ambos, igual que los módulos de Instagram 32/33 de los otros tipos de post, desde 2026-09-21: sin eso, un fallo de Instagram dejaba una ejecución incompleta que trababa la cola (`sequential: true`, `dlq: false`) y apagaba el escenario. Contra: un fallo de Instagram ya no se ve en el historial de Make. Corre 100% fuera de Vercel — no consume Function Invocations ni toca ninguna ruta `/api/` del sitio. Interruptor general + tope diario opcional desde el admin (tab "Noticias (Bot)", `noticias_config/settings`) — el tope diario limita publicaciones exitosas por día, no corridas; con "sin tope" el script igual respeta el máximo de 5 intentos por corrida, repartidos como máximo 2 por tópico para que roten entre los tópicos activos (`noticias_topics`) — sin ese tope por tópico, uno con mucho volumen de RSS (ej. "inteligencia artificial") se comía todo el cupo de la corrida y los demás tópicos activos nunca se publicaban (bug detectado y corregido 2026-09-18). **Scheduling**: el trigger `schedule` de GitHub Actions se sacó del workflow porque demostró ser poco confiable (llegó a saltear varias horas seguidas sin correr) — el disparo real cada hora lo hace un cronjob externo en cron-job.org pegándole a `POST /repos/.../actions/workflows/noticias-bot.yml/dispatches` con un fine-grained PAT (permiso Actions: Read and write, acotado a este repo). El workflow solo tiene `workflow_dispatch` como trigger.
