...
const AnaliticaPlansEditor = forwardRef(function AnaliticaPlansEditor(_props, ref) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500">Solo precio y cuota son editables — los ids (free/básico/full) son fijos, los usa MercadoPago para matchear la suscripción.</p>
      {plans.map(p => {
        const e = edits[p.id] || { price: p.price, limit: p.limit ?? '' };
        return (
          <div key={p.id} className="bg-white dark:bg-neutral-900 rounded-xl border border-gray-200 dark:border-neutral-800 p-4 flex flex-wrap items-center gap-4">
            <div className="min-w-[100px]">
              <p className="font-semibold text-gray-900 dark:text-gray-100">{p.name}</p>
              <p className="text-xs text-gray-500">{fmtARS(p.price)}{p.period === 'month' ? '/mes' : ''}</p>
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 uppercase mb-1">Precio ARS</label>
              <input type="number" min={0} value={e.price} onChange={ev => setField(p.id, 'price', Number(ev.target.value))}
                className="w-28 rounded-lg border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100 px-2.5 py-1.5 text-sm" />
            </div>
            <div>
              <label className="block text-[10px] text-gray-400 uppercase mb-1">Búsquedas (vacío = ilimitado)</label>
              <input type="number" min={0} value={e.limit} onChange={ev => setField(p.id, 'limit', ev.target.value === '' ? '' : Number(ev.target.value))}
                placeholder="∞"
                className="w-32 rounded-lg border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-gray-900 dark:text-gray-100 px-2.5 py-1.5 text-sm" />
            </div>
