    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-gray-500">Vista consolidada de todos los reportes hechos — solo vos la ves, no se publica.</p>
        </div>
        <button onClick={reload}
          className="px-3 py-1.5 text-xs rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700">
          ↻ Recargar
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end bg-gray-50 dark:bg-gray-800/40 rounded-xl p-3">
        <div>
          <label className="block text-[11px] text-gray-500 mb-1">Año</label>
        ...
        <div>
          <label className="block text-[11px] text-gray-500 mb-1">Mes</label>
        ...
        <div>
          <label className="block text-[11px] text-gray-500 mb-1">SEO ≥</label>
        ...
        {hasFilters && (
          <button onClick={clearFilters} className="text-xs text-indigo-500 hover:underline pb-1.5">Limpiar filtros</button>
        )}
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 text-xs">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
            className="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 disabled:opacity-40">← Anterior</button>
          <span className="text-gray-500">Página {page} de {totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
            className="px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 disabled:opacity-40">Siguiente →</button>
        </div>
      )}
