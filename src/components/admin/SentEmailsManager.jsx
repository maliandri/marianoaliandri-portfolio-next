...
const STATUS_STYLE = {
  sent:      { label: 'Enviado',    cls: 'bg-gray-100 dark:bg-neutral-800 text-gray-600 dark:text-gray-300' },
  delivered: { label: 'Entregado',  cls: 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400' },
  opened:    { label: 'Abierto',    cls: 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' },
  clicked:   { label: 'Click',      cls: 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' },
  bounced:   { label: 'Rebotado',   cls: 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' },
  complained:{ label: 'Spam',       cls: 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' },
};

function StatusBadge({ status }) {
  const s = STATUS_STYLE[status] || STATUS_STYLE.sent;
  return <span className={`inline-block text-[11px] font-medium px-2 py-0.5 rounded-full ${s.cls}`}>{s.label}</span>;
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Emails enviados</h2>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="px-3 py-2 text-sm bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-700 rounded-xl text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-400/50"
          />
          <button onClick={load}
            className="px-3 py-2 text-sm border border-gray-300 dark:border-neutral-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors">
            ↻
          </button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-gray-500">
          ...
          {emails.length === 0 && <p className="text-xs mt-1">Enviá uno desde Auditorías → "▼ Más" → Generar.</p>}
        </div>
      ) : (
              ...
              <thead className="bg-gray-50 dark:bg-neutral-900 text-left">
                <tr className="border-b border-gray-200 dark:border-neutral-800">
                  {['Negocio', 'Email', 'Estado', 'Fecha', 'Acciones'].map(h => (
                    <th key={h} className="px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => handleResend(m)} disabled={busy === m.id}
                          className="px-2.5 py-1 text-xs bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg hover:bg-indigo-200 dark:hover:bg-indigo-900/50 transition-colors disabled:opacity-40">
                        ...
                        <button onClick={() => handleDelete(m)} disabled={busy === m.id}
                          className="px-2.5 py-1 text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors disabled:opacity-40">🗑</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setViewing(null)}>
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 w-full max-w-lg max-h-[85vh] overflow-y-auto p-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={() => { handleResend(viewing); setViewing(null); }} className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors">↻ Reenviar</button>
            </div>
          </div>
        </div>
      )}
