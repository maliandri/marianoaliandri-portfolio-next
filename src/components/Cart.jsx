...
import { DEFAULT_PAYMENT_PLAN, sanitizePlan, advanceFraction } from '../data/paymentPlan';

export default function Cart({ onClose }) {
  const { cart, removeFromCart, updateQuantity, clearCart, getCartCount } = useCart();
  const [loading, setLoading] = useState(false);
  const [payAdvance, setPayAdvance] = useState(false); // true = pagar solo la seña
  const [plan, setPlan] = useState(DEFAULT_PAYMENT_PLAN); // plan de pago (editable desde admin)
  const [fx, setFx] = useState({ rate: null, loading: true });
  const fxService = new ExchangeService();
  const ADVANCE_PCT = advanceFraction(plan);          // fracción de la seña (primer hito)
  const advancePctLabel = Math.round(ADVANCE_PCT * 100);

  // Cargar plan de pago configurable
  useEffect(() => {
    fetch('/api/payment-plan')
      .then((r) => r.json())
      .then((d) => { if (d?.plan) setPlan(sanitizePlan(d.plan)); })
      .catch(() => { /* usa default */ });
  }, []);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Si el usuario eligió pagar el adelanto, se cobra el 60% del total como
      // un único ítem "seña". El 20% por avance y el 20% final se coordinan aparte.
      let items;
      if (payAdvance) {
        const advance = Math.ceil(getCartTotal() * ADVANCE_PCT);
        const resumen = cart.map(i => `${i.quantity}× ${i.name}`).join(', ');
        const restantes = plan.milestones.slice(1).map(m => `${m.pct}% ${m.label}`).join(' + ');
        items = [{
          title: `Adelanto ${advancePctLabel}% (seña)`,
          description: `Seña del ${advancePctLabel}% — ${resumen}.${restantes ? ` Restante: ${restantes} a coordinar.` : ''}`,
          unit_price: advance,
          quantity: 1,
          currency_id: 'ARS',
        }];
      ...
      const preference = await createPaymentPreference({
        ...
        metadata: {
          ...
          payment_mode: payAdvance ? 'advance_30' : 'full',
          cart_total_ars: getCartTotal(),
          ...
          {cart.length > 0 && (
            <div className="border-t border-gray-200 dark:border-gray-700 p-4 space-y-4 bg-gray-50 dark:bg-gray-900">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  Adelanto {advancePctLabel}%
                </button>
              </div>

              {/* Desglose del plan de pago */}
              {payAdvance && (
                <div className="rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 p-3 text-sm space-y-1">
                  {plan.milestones.map((m, i) => (
                    <div
                      key={i}
                      className={`flex justify-between ${
                        i === 0 ? 'text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'
                      }`}
                    >
                      <span>
                        {m.label} ({m.pct}%){i === 0 ? ' — pagás ahora:' : ':'}
                      </span>
                      <span className={i === 0 ? 'font-bold text-purple-700 dark:text-purple-300' : ''}>
                        {formatARS(Math.ceil(getCartTotal() * (m.pct / 100)))}
                      </span>
                    </div>
                  ))}
                  <p className="text-xs text-gray-500 dark:text-gray-400 pt-1">
                    Solo se cobra online la seña. El resto se coordina una vez confirmada.
                  </p>
                </div>
              )}
